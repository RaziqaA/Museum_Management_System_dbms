...
if(!empty($logged_in_id)){
	...
	if(mysqli_connect_error()){
		...
		$UPDATE = "UPDATE staff SET entry_time = ? WHERE staffid = ? LIMIT 1";
		
		$stmt = $conn->prepare($UPDATE);
		$stmt->bind_param("ss", $date, $logged_in_id);
		$stmt->execute();
		$_SESSION['msg'] = "<script>window.alert('Entry Date and Time updated successfully')</script>";
	...
	echo "<script>window.alert('Login ID not present')</script>";
	header("location: employee_menu.php");
}
?>